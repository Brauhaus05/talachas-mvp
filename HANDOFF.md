...
Done (11/11 features; reviews loop = **Phase 6 cycle 1**, PR #13 **merged** 2026-07-07): auth, profiles, KYC (Connect), search/filter, availability slots, booking + concurrency, payments/commission/tips, **1:1 chat**, **transactional email**, **reviews loop**. **Admin panel (cycle 2) built — PR open. Next work: Phase 6 cycle 3 — disputes queue.**

| Phase | Scope | Size |
|---|---|---|
| **6 cycle 3 — Disputes queue** | admin-mediated disputes: a client/talachero flags a booking, it lands in an admin queue, admin resolves (force-refund is already built in cycle 2). **Needs a flag mechanism — none exists today** (new `disputes` table + flag action + status enum). Pairs with the **cancellation-policy tiers** (partial/tiered refunds) so `refundCapturedBooking` can take an amount. | ~1.5 d |
| **Deferred 5B email** | **24h reminder** (needs a scheduler/cron). New-review email done in cycle 1. | folds into 6 |
...
**Recommended order:** first **merge the cycle-2 admin PR** (open now), then ▶ **START Phase 6 cycle 3 (disputes queue)** — its own spec + plan + build. The force-refund resolution action already exists (cycle 2), so cycle 3 is mostly the flag mechanism + queue UI. Slot the deferred talachero self-service tooling in before onboarding real talacheros.
...
> Continuing Talachas. Phases 0–4B, **5A, 5B, 6 cycle 1 (reviews loop) all merged**; **6 cycle 2 (admin panel) built on `feat/phase6-admin-panel` with a PR open** (2026-07-07): 5 admin RPCs (`admin_set_ban` + curated `admin_list_users/bookings/reviews` + `admin_delete_review`), ban-aware directory/booking, seed admin `admin@talachas.mx`/`password123`, and `/dashboard/admin` users(ban)/bookings(**force-refund** via the shared `refundCapturedBooking` helper)/reviews(delete) + overview. Verified at the **DB layer** (every RPC end-to-end incl. `cannot_ban_admin` + non-admin `not_authorized`), **build/typecheck/lint green**, signed-out route guards **307**. Owner runbooks still open: **live force-refund** (Stripe test keys + re-onboarded talachero + captured booking) and the **admin UI browser pass** (sign in as `admin@talachas.mx`); the cycle-1 review UI pass is also still un-eyeballed (fresh booking `9a28d83f` primed). **Next work: merge the cycle-2 PR, then START Phase 6 cycle 3 — disputes queue** (needs a new flag mechanism — none exists; pairs with cancellation-policy tiers). The **24h reminder email** (needs cron) remains deferred. `CLAUDE.md` is the fast architecture read.
