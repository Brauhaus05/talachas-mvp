...
- **Flow:** new talachero (`pending`) completes **profile** (rate + ≥1 service) + **availability** (≥1 upcoming slot) → **submit for review** (`in_review`) → admin **approve** (`verified` → listed) or **reject** (`rejected` + reason → fix → resubmit). **Stripe stays a parallel step** (needed to receive *paid* bookings, not to get listed).
- **State model:** `verification_status` enum gains **`in_review`** (`pending | in_review | verified | rejected`) + new `talachero_profiles` columns `rejection_reason` / `submitted_at`. Two migrations: `20260722130001` (enum add-value, isolated — the Postgres same-txn gotcha) + `20260722130002` (columns + 3 `SECURITY DEFINER` RPCs `submit_talachero_for_review` / `admin_list_verifications` / `admin_review_talachero` with `FOR UPDATE` + `not_in_review` guard).
- **⚠️ Behavioral change — Stripe decoupled from verification.** The `account.updated` webhook + `refreshOnboarding` **no longer set `verification_status`** (they only write `charges_enabled`/`payouts_enabled`); admin approval is now the **sole** path to `verified`. Existing `verified` talacheros (seed) are untouched (no downgrade path).
- **UI:** talachero dashboard **`OnboardingChecklist`** card (3 steps + submit/resubmit + in_review/rejected/live states) reusing the existing editors; admin **`/dashboard/admin/verifications`** queue (name/bio/services/rate/slots/payments badge + Approve/Reject-with-reason) + **5th admin overview card**. New `onboarding` + `admin` (verification) i18n keys, es/en in sync.
- **Verified:** typecheck/lint/secretless build green (both new routes in the build list) + **DB-level RPC state machine** (submit guards `profile_incomplete`/`no_availability`/`already_submitted`, approve→`verified`, reject→`rejected`+reason/`empty_reason`, `not_in_review`, non-admin `not_authorized`) + **live talachero browser pass** (checklist renders with correct step states → submit → **En revisión** banner + submit hidden; DB shows `in_review`; **directory excludes `in_review`** — `list_talacheros` still filters `= 'verified'`; zero console errors). **Admin-side queue browser walk-through is an owner runbook** (needs admin sign-in) — **Carlos is left `in_review` on the local DB to prime it**: sign in `admin@talachas.mx` → **Verificaciones** → approve/reject.
- **🔎 Design note / follow-ups (not bugs):** (a) **`verified` no longer implies *payable*** — an admin can approve before Stripe is done, so a talachero can be listed yet `confirmBooking` returns `talachero_not_payable` until they finish Stripe. This is the accepted "Stripe is parallel" decision and **already true for seed talacheros**; the admin queue shows a `payments_ready/pending` badge for judgment. Optional future tightening: also filter `list_talacheros` on `charges_enabled`, or require it at approval. (b) `admin_review_talachero` re-checks only `status='in_review'`, not completeness, so approving a since-stripped profile is possible (admin sees live `slot_count`). (c) admin overview is `lg:grid-cols-4` with 5 cards now (4+1 wrap — cosmetic).
- **Cloud:** both migrations (`20260722130001` + `20260722130002`) must be pushed via `supabase db push` (through the us-east-1 pooler `--db-url`) before this works in production — same as the prior push.
...
**PR #19 — ✅ MERGED to `main` (squash `33fac07`):** first talachero **self-service availability editor** — closes Sprint 2 **"Editor de disponibilidad (horarios semanales y excepciones)"** (Notion: Hecho). Built subagent-driven from a committed spec + plan in `docs/superpowers/` (`2026-07-22-talachero-availability-editor-*`), each task through spec + code-quality review, plus a final holistic review (**ready to merge**, no Critical/Important).
...
- **☁️ Cloud DB migrated (2026-07-22):** both `20260716120001_update_talachero_profile.sql` (profile editor, was never pushed after PR #18) **and** `20260722120001_availability_editor.sql` were applied to the cloud project `rcpfxcwooptmadyacfkk` via `supabase db push` (through the us-east-1 pooler — the direct `db.<ref>` connection is IPv6-only and times out on most networks; use `--db-url "...pooler.supabase.com..."`). Cloud is now migrated through `20260722120001` — **`db push`, never `db reset --linked`** (a reset wipes talachero Stripe onboarding). Vercel prod (`33fac07`) carries both editors.
